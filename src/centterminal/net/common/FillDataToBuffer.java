...
public class FillDataToBuffer {
    ...
    public static byte[] load(Object DB, int buffeSize) {
        ...
        int intLength =4; // A "int" típusu adat hossza
        ...
        for (Field field : fields) {
            try {
                ...
                if (s.equals("short")) {
                    ...
                    ByteBuffer.wrap(sendTelegram, pointer, longLength).putLong(field.getLong(DB));
                    pointer += longLength;
                
                } else if(s.equals("int")){
                    ByteBuffer.wrap(sendTelegram, pointer, intLength).putInt(field.getInt(DB));
                    pointer += intLength;
                }else {
